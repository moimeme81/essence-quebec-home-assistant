...
class RegieEssenceCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        try:
            # Fetch all stations once per update cycle
        ...
        # Map them by their exact address so the sensors can instantly find themselves
        return {s.get("Address"): s for s in all_stations if s.get("Address")}
